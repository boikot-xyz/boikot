...
export async function testGetWikipediaPage() {
    targetWikipediaPages.forEach( async ([companyName, targetUrl]) => {
        const url = await getWikipediaPage(companyName);
        if( url !== targetUrl ) console.log(url, targetUrl);
    });
}
...
export async function testGetWikipediaInfo() {
    targetWikipediaInfos.forEach( async ([companyName, target]) => {
        const wikipediaInfo = await getWikipediaInfo(companyName);
        if( !_.isEqual(wikipediaInfo, target) ) console.log(wikipediaInfo, target);
    });
}


async function runTests() {
    await testGetWikipediaPage();
    await testGetWikipediaInfo();
}
